  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Your complete check-in record across all courses.</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700 shadow-sm text-center">
          <p className="text-3xl font-bold text-red-600">{attendanceRate}%</p>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {records.map((record) => (
              <div key={record.id} className="p-5 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                <div className={`p-2.5 rounded-xl shrink-0 ${record.status === "present" ? "bg-blue-50 text-blue-600" : "bg-slate-100 text-slate-400 dark:bg-slate-700"}`}>
