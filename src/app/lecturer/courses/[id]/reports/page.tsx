...
import { getLecturerCourses, getUsersByRole } from "@/lib/firebase/firestore";
import { collection, query, where, getDocs, orderBy } from "firebase/firestore";
import { db } from "@/lib/firebase/config";
import { Course, AttendanceSession, AttendanceRecord } from "@/lib/mock/db";
import { ArrowLeft, Download, FileText, Sheet, Calendar, Users, Clock } from "lucide-react";
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {sessions.map((session) => (
            <div key={session.id} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 shadow-xl text-white flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div>
              ...
              <div className="flex items-center gap-3 sm:ml-auto">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 bg-white text-blue-900 rounded-xl font-bold hover:bg-blue-50 transition-all hover:scale-105 active:scale-95 shadow-md text-sm"
                >
